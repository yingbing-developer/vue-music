<template>
  <div class="player" v-show="playlist.length > 0">
    ...
    <audio ref="audio" hidden @play="ready" @error="error" @timeupdate="updateTime" @ended="end"></audio>
    <p hidden v-html="lyric" ref="lyric"></p>
  </div>
</template>
...
export default {
    ...
    methods: {
        ...
        loop() {
          if(this.playlist.length === 1 && !this.playing) {
            this.setPlayingState(!this.playing)
          }
        ...
        getLyric() {
          this.currentSong._getLyric().then((lyric) => {
            ...
            this.lyric = lyric
            this.$nextTick(() => {
              const lrc = this.$refs.lyric.innerHTML
        ...
        getPlayUrl() {
          this.currentSong._getPlayUrl().then((playUrl) => {
            if(!playUrl) {
